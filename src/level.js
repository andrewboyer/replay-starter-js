...
export const Level = makeSprite({
  ...
  loop({ props, state, device }) {
    ...
    if( lastPipe.x < 140 ){
      ...
      // remove pipes to the left of the screen
      .filter(
        (pipe) =>
  ...
  // render does things in order.
  // so, can we pass a data structure to this that has the layers?
  // that way, you can do background, etc
